...
C***********************************************************************
     ...
     $ ,KODAT(MAXATOM),NFIRST(MAXATOM),NLAST(MAXATOM),IADR(MAXADR)
     $ ,INDNUP(MAXIND),INDLOW(MAXIND),MODHIST(MAXHIST)
...
!     DECODING INPUT DATA
     ...
     $               LBKG,XLBKG1,XLBKG2,
     $               TPLOT,MAXATOM,NATOM,ABXYZ,KODAT,IDAT,LBLANK,ATMEAN)
     ...
     $                   INDLOW,INDNUP)
      ...
      CALL       FGRID (NFDIM,NF,XLAMBDA,FWEIGHT,AKEY,NOM,SYMBOL,NATOM,
     $                  N,NCHARG,ELEVEL,EION,EINST,NDIM )
C
      !*** 2.1) Create the Geometrical Mesh (sperical)
      CALL GEOMESH (RADIUS,ENTOT,RHO,T,FAL,AINCRIT,P,Z,RMAX,
...
c     JSTART writes the files RADIOC and RADIOL
     ...
     $            NCHARG,ELEVEL,EDDI,WCHARM,NOM,N,EINST,NDIM,
     $            MODHEAD,JOBNUM,TEFF)
