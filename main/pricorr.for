     ...
     $                   TNEW, NOTEMP, DELTAC, ELEMENT, NATOM, NFIRST, NLAST)
...
!     RELATIVE TO THE LAST ITERATION
      ...
      DIMENSION NCHARG(N), RNE(ND), T(ND), TNEW(ND)
...
C***  PRINTOUT OF THE TEMPERATURE CORRECTIONS
      IF (.NOT. NOTEMP .AND. LSPOP .GT. 0) THEN
      PRINT 21
...
C***  MAXIMUM TEMPERATURE CORRECTION
      ...
      IF (ABS(RTC) .GT. ABS(RTCMAX)) THEN
         ...
         ENDIF
   24 CONTINUE
      PRINT 25,RTCMAX,LRTCMAX
   25 FORMAT (/,15X,'MAX.REL.TEMP.CORR.=',F8.4,'  AT L=',I3)
      ENDIF
     
      IF (GAMMAL.GT..0)
     $   PRINT 14, GAMMAL,DELTAC,GAMMAR
   14    FORMAT (/,10X,'SCHARMER-PARAMETERS:',  10X,
     $          '   GAMMAL=',F5.1,/,
     $        40X,'DELTAC=',F5.1,'   GAMMAR=',F7.1)
