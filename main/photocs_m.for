...
C     print 5,'PHOTOCS: AGAUNT(LOW)',low,agaunt(low)
C    5 FORMAT(A20,I2,A10)
      X=EDGE/WAVENUM
...
cpr         print *,' f-lam: ',xlam,flamda/x,SIGMA
      ELSE
C***  OLD VERSION: SEATON / HYDROGENIC
cpr         print *,' old branch'
...
cpr         print *,' Mihalas branch'
            GAUNT=A0+X*(AM1+X*AM2)+(A1+(A2+A3/X)/X)/X
            SIGMA=SIGMA*GAUNT
c         print *,low,'MIHALAS: SIGMA=',SIGMA
            ENDIF     
...
cpr         print *,' Seaton branch'
            IF (SEXPO(LOW) .LE. .0d0) THEN
               print *, 'PHOTOCS_HM: MAIN QUANTUM NUMBER UNDEFINED'
               STOP 'ERROR'
               ENDIF
            U=one/X - one
     ...
     -            0.0496d0 * (U*(U+1.333333333333d0)+one) * DEN * DEN
            SIGMA=SIGMA*GAUNT
c     print *,low,'SEATON: SIGMA=',SIGMA
            ENDIF    
      RETURN
      END SUBROUTINE
      END MODULE
